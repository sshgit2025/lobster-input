...
## 8. 2026-05-28 快捷键间歇失效回归记录

### 8.1 现象

- 套餐和支付模块迭代后，安装版 App 出现全局快捷键用一段时间后完全不触发的问题。
- 不是永久不可用：进入设置页修改或重新保存快捷键后，又能恢复触发。
- 本机日志中能看到修改快捷键后 `HotKey: event tap installed`，随后快捷键恢复。

### 8.2 回归原因

`feat: update mac subscription checkout flow` 中误混入了 `HotKeyManager.swift` 的 App 层 event tap 恢复逻辑：

- `NSWorkspace.didWakeNotification`
- `NSApplication.didBecomeActiveNotification`
- 5 秒 `Timer` health check
- `recoverEventTapIfNeeded`
- `reinstallEventTap`

这套逻辑把快捷键监听从原本的单一生命周期改成了多入口重启。它会和 `PermissionManager.refreshStatuses()`、设置页快捷键录制流程、录音/实时识别状态切换同时操作同一个 `CGEventTap`，容易让 `isListening`、`tapPort`、`retainedSelf`、运行时按键状态之间出现短暂不一致。用户感知就是：监听看起来仍处于开启状态，但实际快捷键不再触发；修改快捷键会调用 `stopListening()` + `startListening()`，所以又恢复。

### 8.3 修复原则

- 回到原来的稳定策略：`HotKeyManager` 只负责一条明确的 `startListening` / `stopListening` 生命周期。
- 不在 App 激活、系统唤醒、定时器中额外重装 event tap。
- 保留 `.defaultTap` 同步拦截、线程安全配置快照、keyDown/keyUp 配套吞键。
- `tapDisabledByTimeout` / `tapDisabledByUserInput` 只做原地 `CGEvent.tapEnable`，不跨层重装监听器。
- 如果未来确实要做 监听状态监测器，必须作为 HotKey 子系统内部的一体化设计落地，提供单一状态源和显式 `resetPressingKeycodes()`，不要分散到权限、窗口生命周期和业务流程里。

### 8.4 2026-05-28 二次回归修复

后续测试再次出现“运行一段时间后快捷键完全不触发，修改快捷键后恢复”的现象。复查发现上一轮只删除了外层 watcher/restart 逻辑，但主触发仍依赖 `CGEventTap`，并且 `PermissionManager.refreshStatuses()`、权限窗口关闭等路径仍会直接重装监听器，实际没有做到单一生命周期。

本次改为更接近 macOS 标准全局快捷键注册表的方案：

- 普通快捷键（默认 `Option+Q/E/W/A` 及其它非 Fn、非纯修饰键组合）使用 Carbon `RegisterEventHotKey` 注册，由系统热键表负责触发。
- `CGEventTap` 不再作为普通快捷键主触发路径，只在业务上确实需要的 Fn/纯修饰键快捷键中作为 fallback 使用。
- `PermissionManager.refreshStatuses()` 只刷新权限状态，不再启动、停止或重装快捷键监听。
- 权限窗口关闭只刷新权限，不再无条件 `startListening()`。
- App 启动时显式调用一次 `HotKeyManager.startListening`；用户切换后 `reloadForCurrentUser()` 会在已监听状态下重载配置并重建注册表。

设计原则更新：

- 默认路径优先使用系统注册表，而不是高频事件过滤器。
- `CGEventTap` 只用于 Carbon 表达不了的快捷键形态，不作为普通快捷键稳定性的基础。
- 任何权限刷新、窗口生命周期、业务状态变化都不能私自重装快捷键监听；配置变更和用户切换是允许重建注册表的明确边界。

### 8.6 2026-06-03 token 过期导致监听被停用

本机运行日志再次出现“运行中快捷键全部失效，修改快捷键后恢复”。日志显示最后一次 Carbon 回调发生在实时 ASR 连接失败后的停止处理流程中，随后后端返回 `登录已过期，请重新登录`，业务调用 `AuthStore.logout()`；之后 App 仍继续执行预热定时任务，但不再有任何 `HotKey CARBON` 记录。

根因是 `logout()` 内部主动调用了 `HotKeyManager.stopListening()`，并假设登录界面的 `MainView.onAppear` / 权限刷新会重新启动监听。实际退出登录或 token 过期后可能停留在登录态切换界面，不会走稳定的重新注册路径，所以系统热键注册被清空，直到用户修改快捷键触发设置页 `restartHotKeyListening()` 才恢复。

修复原则：

- 退出登录、token 过期、套餐/账号状态变化都属于业务登录态变化，不能停止 App 级全局快捷键监听。
- 未登录状态由 `AppHotKeyHandler` / `RealtimeRecordingWorkflow` 入口拦截，不通过注销 Carbon 热键实现。
- 用户切换账号时，登录成功的 `AuthStore.save()` 仍通过 `HotKeyManager.reloadForCurrentUser()` 切换账号级快捷键配置；这属于配置边界，可以重建注册表。

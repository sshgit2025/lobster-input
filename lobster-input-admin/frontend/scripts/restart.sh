...
echo "🦞 Lobster Admin Frontend - 构建并启动预览..."
...
PREVIEW_PORT="${PREVIEW_PORT:-7888}"
...
for _ in {1..30}; do
  ...
  if curl -fsS "http://127.0.0.1:${PREVIEW_PORT}/lobster/admin/" >/dev/null 2>&1; then
    echo "✅ 前端预览已启动，PID=${NEW_PID}"
    echo "📌 访问地址: http://localhost:${PREVIEW_PORT}/lobster/admin/"

...
class SubscriptionLifecyclePolicy:
    ...
    def snapshot_from_user(self, user: dict) -> SubscriptionSnapshot:
        return SubscriptionSnapshot(
            plan_code=user.get("subscription_plan_code") or user.get("plan_code") or user.get("tier") or "",
    ...
    def linked_grant_action(self, snapshot: SubscriptionSnapshot, now: datetime) -> LinkedGrantAction:
        if snapshot.plan_code and snapshot.plan_code != self.free_plan_code and snapshot.expires_at and as_utc(snapshot.expires_at) > now:
            return LinkedGrantAction.SYNC
        if snapshot.plan_code in {"", self.free_plan_code}:
...
class CreditGrantLifecycleCoordinator:
    ...
    async def apply_subscription_activation(
        ...
        *,
        ...
        if snapshot.plan_code in {"", FREE_PLAN_CODE}:
    ...
    async def sync_subscription_linked_grants(self, email: str, plan_code: str, expires_at: datetime, now: datetime) -> int:
        result = await self.db["credit_grants"].update_many(
            {"user_email": email, "status": CREDIT_STATUS_ACTIVE, "metadata.expires_at_mode": SUBSCRIPTION_EXPIRES_MODE},
            {"$set": {
                "expires_at": as_utc(expires_at),
                "metadata.subscription_plan_code": plan_code,
                "metadata.subscription_expires_at": as_utc(expires_at),
                "updated_at": now,
            }},
        )
        return result.modified_count
    ...
    async def extend_paid_topups(self, email: str, expires_at: datetime, now: datetime) -> int:
        result = await self.db["credit_grants"].update_many(
            {"user_email": email, "credit_type": PAID_TOPUP_CREDIT_TYPE, "status": CREDIT_STATUS_ACTIVE, "$expr": {"$lt": ["$amount_used", "$amount_total"]}},

# 鸿蒙端发布文档 — preview(内测环境)

每次发 preview 版本照本文档从上往下执行。uat(公测环境,正式版)发布见 `RELEASE-uat.md`,两环境内容不混用。

## 〇、环境信息

| 项 | 值 |
|---|---|
| 环境标识 | `preview`(内测环境,新建) |
| API 地址 | `https://api.example.net/lobster/` |
| 更新 feed | `https://downloads.example.com/preview/harmony/update.json` |
| 客户端徽章 | 内测版(Beta),由 `ApiConfig.ENVIRONMENT` 派生 |
| 包名 | `com.lobster.input`(与 uat 相同,靠 versionCode 区分) |
...
- 本轮 preview 版本映射:`versionName = 1.0.103`,`versionCode = 108`(uat 为 0.0.1/107)。
- 版本映射维护在 `scripts/release.sh` 顶部,发新版时更新脚本里的映射(或用环境变量 `VERSION_NAME`/`VERSION_CODE` 覆盖);仓库里 `AppScope/app.json5` 由脚本临时改写并自动恢复,不需要手改。

## 三、发新版本流程

1. **确认版本映射**:检查 `scripts/release.sh` 中 preview 的 `VERSION_NAME`/`VERSION_CODE` 是本次要发的号(versionCode 必须比两环境线上任何已用号都大)。
2. **一键打包**:
   ```bash
   scripts/release.sh preview   # preview 是默认环境,不带参数等价
   ```
   脚本自动完成:临时把 `ApiConfig.ets` 的 `ACTIVE_ENV` 切到 `preview`、临时改 `AppScope/app.json5` 版本号 → release 编译未签名 App Pack → `hap-sign-tool` 对 **.app 整包**签名 → 恢复被临时修改的源文件(失败也会恢复)。
   产物:`dist/lobster-input-<版本号>-preview-signed.app`。
...
3. **上传 AGC**:[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html) → 我的应用 → 龙虾输入法 → 版本信息 → 上传 `.app` 包 → 填更新说明 → 提交审核。
   - **同一 bundleName 两环境包并存**:preview 与 uat 包名相同,仅 versionCode 不同;把哪个环境的包投到哪个通道(正式发布 / 邀请测试)由发布人自行选择。preview 是内测包,常规走「邀请测试」灰度通道,勿提交正式上架。
   - **邀请测试**(灰度):左侧「测试」→ 邀请测试 → 维护测试用户华为账号名单 → 提交测试审核,通过后测试用户收到链接安装。
4. **更新 feed**:发布后更新 `preview/harmony/update.json`(R2,bucket=lobster-mac),内容为本次 versionName/versionCode;**此 feed 永不指向 uat 的包/清单,也永不写入老用户旧 feed(`android/preview/update.json`,那是 uat 迁移桥专用)**。

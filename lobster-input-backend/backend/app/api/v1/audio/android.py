"""
Android 端音频处理接口。
POST /api/v1/audio/android/process — Android 专属语音识别接口。

平台标识：
  client_platform 固定为 "android"（接口层强制注入）
  流程选择：由 PipelineManager 根据 X-Accept-Language Header 自动路由

功能说明：
  - transcribe / rewrite 已按移动端纯语音输入法流程支持
  - 文本快捷处理不属于音频流程，见 /api/v1/text/android/quick-action
  - Android 输入法暂不支持 agent / OpenClaw / 搜索类桌面能力
"""
...
router = APIRouter(prefix="/audio/android", tags=["Audio - Android"])
...
@router.post(
    ...
    summary="Android 端：上传音频文件并返回处理结果",
    description="""
Android 专属语音识别接口，提示词由 PipelineManager 根据语言自动路由。

支持的 operation：
- transcribe — 语音转文字 + 纠偏整理
- rewrite    — 语音指令改写选中文本

X-Client-Platform 无需传递，接口层强制注入 android。
X-Accept-Language 由客户端传递当前语言类型，用于流程选择和提示词路由。
""",
)
@with_client_context(client_platform=_ANDROID_PLATFORM)
async def android_process_audio(

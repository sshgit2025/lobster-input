"""Mac v2 text processing and realtime ASR endpoints."""
...
from app.services.pipeline.v2.text_pipeline import MAC_V2_PIPELINES

logger = logging.getLogger("voice_input.api.v2.audio_mac")

router = APIRouter(prefix="/audio/mac", tags=["Audio v2 - Mac"])

_MAC_PLATFORM = "macos"
_MAC_ALLOWED_OPERATIONS = {"transcribe", "rewrite", "agent"}
_MAC_SESSION_ID_RE = re.compile(r"^[A-Za-z0-9_.:-]{1,96}$")


@router.post(
    "/process",
    response_model=AudioTranscribeResponse,
    summary="Mac v2：处理实时 ASR 最终文本",
    description="Mac 专属 v2 接口，接收实时 ASR 最终文本并执行 Mac 业务管线。",
)
@with_client_context(client_platform=_MAC_PLATFORM, flow_name="v2")
async def mac_process_realtime_text(
    payload: TextProcessRequest,
    client_context: ClientRequestContext,
) -> AudioTranscribeResponse:
    operation = (payload.operation or "").strip().lower()
    if operation not in _MAC_ALLOWED_OPERATIONS:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Mac 端仅支持 transcribe、rewrite 和 agent",
        )

    resolved_text, resolved_language, asr_resolution_source = await _resolve_mac_realtime_transcript(
        payload=payload,
        user_email=client_context.user_email,
    )
    pipeline = MAC_V2_PIPELINES.get(operation)
...
@router.post(
    "/process/stream",
    summary="Mac v2：处理实时 ASR 最终文本并流式返回搜索结果",
    description="仅联网搜索节点会输出 delta；非搜索 agent 只返回 final 事件。",
)
@with_client_context(client_platform=_MAC_PLATFORM, flow_name="v2")
async def mac_process_realtime_text_stream(
    payload: TextProcessRequest,
    client_context: ClientRequestContext,
):
    operation = (payload.operation or "").strip().lower()
...
async def mac_realtime_asr(websocket: WebSocket) -> None:
    try:
        auth_payload = await _verify_mac_websocket_user(websocket)
    except Exception as exc:
        logger.warning("[mac v2 realtime_asr] auth failed: %s", exc)
    ...
    try:
        asr_session_id = _normalize_mac_session_id(query.get("asr_session_id") or "")
    ...
    cfg = RealtimeASRConfig(
        ...
        audio_format=_normalize_mac_audio_format(query.get("audio_format") or query.get("format") or "pcm"),
        sample_rate=_normalize_mac_sample_rate(query.get("sample_rate")),
        use_vad=_parse_mac_bool(query.get("vad"), default=True),
        threshold=_normalize_mac_float(query.get("threshold"), default=0.0, min_value=-1.0, max_value=1.0),
        silence_duration_ms=_normalize_mac_int(
            query.get("silence_duration_ms"), default=400, min_value=200, max_value=6000,
        ),
        max_duration_sec=_normalize_mac_int(
...
async def _resolve_mac_realtime_transcript(
    *,
    payload: TextProcessRequest,
    user_email: str,
) -> tuple[str, str, str]:
    ...
    asr_session_id = _normalize_mac_session_id(payload.asr_session_id or "")
    if not asr_session_id:
    ...
    if existing and (
        existing.get("user_email") != user_email or existing.get("platform") != _MAC_PLATFORM
    ):
    ...
    if session and session.get("status") == "completed":
        ...
        if final_text:
            logger.info(
                "[mac v2 audio] ASR session final resolved session=%s final_chars=%d "
        ...
        logger.warning("[mac v2 audio] ASR session completed with empty final: %s", asr_session_id)
        return fallback_text, fallback_language, "client_fallback_empty_server_final"
    elif session and session.get("status") == "failed":
        logger.warning("[mac v2 audio] ASR session failed, using client fallback: %s", asr_session_id)
        return fallback_text, fallback_language, "client_fallback_session_failed"
    else:
        logger.info(
            "[mac v2 audio] ASR session final wait timed out, using client fallback session=%s fallback_chars=%d",
            asr_session_id,
            len(fallback_text),
        )
...
async def _verify_mac_websocket_user(websocket: WebSocket) -> dict[str, Any]:
    """WebSocket 鉴权：五端共用逻辑（含滑动窗口过期校验与异步续期）见 session_service。"""
    return await verify_websocket_user(websocket, _MAC_PLATFORM)


def _normalize_mac_session_id(value: str) -> str:
    normalized = str(value or "").strip()
    if not normalized:
        return ""
    if not _MAC_SESSION_ID_RE.fullmatch(normalized):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid ASR session id")
    return normalized


def _normalize_mac_audio_format(value: str) -> str:
    normalized = str(value or "pcm").strip().lower()
    return normalized if normalized in {"pcm", "opus"} else "pcm"


def _normalize_mac_sample_rate(value: str | None) -> int:
    try:
...
def _parse_mac_bool(value: str | None, *, default: bool) -> bool:
    if value is None:
        return default
    return str(value).strip().lower() in {"1", "true", "yes", "on"}


def _normalize_mac_int(value: str | None, *, default: int, min_value: int, max_value: int) -> int:
    try:
...
def _normalize_mac_float(value: str | None, *, default: float, min_value: float, max_value: float) -> float:
    try:

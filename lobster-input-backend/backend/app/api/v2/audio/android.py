"""Android v2 text processing and realtime ASR endpoints."""
...
logger = logging.getLogger("voice_input.api.v2.audio_android")

router = APIRouter(prefix="/audio/android", tags=["Audio v2 - Android"])

_ANDROID_PLATFORM = "android"
_ANDROID_ALLOWED_OPERATIONS = {"transcribe", "rewrite"}
_ANDROID_SESSION_ID_RE = re.compile(r"^[A-Za-z0-9_.:-]{1,96}$")


@router.post(
    "/process",
    response_model=AudioTranscribeResponse,
    summary="Android v2：处理实时 ASR 最终文本",
    description="Android 专属 v2 接口，接收实时 ASR 最终文本并执行 Android 输入法业务管线。",
)
@with_client_context(client_platform=_ANDROID_PLATFORM, flow_name="v2")
async def android_process_realtime_text(
    payload: TextProcessRequest,
    client_context: ClientRequestContext,
) -> AudioTranscribeResponse:
    operation = (payload.operation or "").strip().lower()
    if operation not in _ANDROID_ALLOWED_OPERATIONS:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Android 输入法仅支持 transcribe 和 rewrite",
        )

    resolved_text, resolved_language = await _resolve_android_realtime_transcript(
        payload=payload,
...
async def android_realtime_asr(websocket: WebSocket) -> None:
    try:
        auth_payload = await _verify_android_websocket_user(websocket)
    except Exception as exc:
        logger.warning("[android v2 realtime_asr] auth failed: %s", exc)
    ...
    try:
        asr_session_id = _normalize_android_session_id(query.get("asr_session_id") or "")
    ...
    cfg = RealtimeASRConfig(
        ...
        audio_format=_normalize_android_audio_format(query.get("audio_format") or query.get("format") or "pcm"),
        sample_rate=_normalize_android_sample_rate(query.get("sample_rate")),
        use_vad=_parse_android_bool(query.get("vad"), default=True),
        threshold=_normalize_android_float(query.get("threshold"), default=0.0, min_value=-1.0, max_value=1.0),
        silence_duration_ms=_normalize_android_int(
            query.get("silence_duration_ms"), default=400, min_value=200, max_value=6000,
        ),
        max_duration_sec=_normalize_android_int(
...
async def _resolve_android_realtime_transcript(
    *,
    payload: TextProcessRequest,
    user_email: str,
) -> tuple[str, str]:
    ...
    asr_session_id = _normalize_android_session_id(payload.asr_session_id or "")
    if not asr_session_id:
    ...
    if existing and (
        existing.get("user_email") != user_email or existing.get("platform") != _ANDROID_PLATFORM
    ):
    ...
    if session and session.get("status") == "completed":
        ...
        if final_text:
            logger.info(
                "[android v2 audio] ASR session final resolved session=%s final_chars=%d "
        ...
        logger.warning("[android v2 audio] ASR session completed with empty final: %s", asr_session_id)
    elif session and session.get("status") == "failed":
        logger.warning("[android v2 audio] ASR session failed, using client fallback: %s", asr_session_id)
    else:
        logger.info(
            "[android v2 audio] ASR session final wait timed out, using client fallback session=%s fallback_chars=%d",
            asr_session_id,
            len(fallback_text),
        )
    return fallback_text, fallback_language
...
async def _verify_android_websocket_user(websocket: WebSocket) -> dict[str, Any]:
    """WebSocket 鉴权：五端共用逻辑（含滑动窗口过期校验与异步续期）见 session_service。"""
    return await verify_websocket_user(websocket, _ANDROID_PLATFORM)


def _normalize_android_session_id(value: str) -> str:
    normalized = str(value or "").strip()
    if not normalized:
        return ""
    if not _ANDROID_SESSION_ID_RE.fullmatch(normalized):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid ASR session id")
    return normalized


def _normalize_android_audio_format(value: str) -> str:
    normalized = str(value or "pcm").strip().lower()
    return normalized if normalized in {"pcm", "opus"} else "pcm"


def _normalize_android_sample_rate(value: str | None) -> int:
    try:
...
def _parse_android_bool(value: str | None, *, default: bool) -> bool:
    if value is None:
        return default
    return str(value).strip().lower() in {"1", "true", "yes", "on"}


def _normalize_android_int(value: str | None, *, default: int, min_value: int, max_value: int) -> int:
    try:
...
def _normalize_android_float(value: str | None, *, default: float, min_value: float, max_value: float) -> float:
    try:
